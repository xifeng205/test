...
class Videoprocessor {
    ...
    public:
        Videoprocessor():call_it(true), delay(0), fnumber(0),
        stop(false), frame_to_stop(-1), is_video_write(false) {}
        ...
        void set_frame_processor(
                Frameprocessor *callback_process) {
            process = NULL;
        ...
        void run() {
            ...
            stop = false;
            while (!is_stopped()) {
                if (!read_next_frame(frame)) break;
                
                if (window_name_input.length() != 0) 
                    cv::imshow(window_name_input, frame);

                if (call_it) {
                    if (frameprocess) frameprocess->process(frame, output);
                    else if (process) process(frame, output);
                    fnumber++;
                } else 
                    output = frame;
        ...
        void get_frame(cv::Mat &frame, cv::Mat &output) {
            if (!is_opened()) return;
            if (!read_next_frame(frame)) return;
        ...
        void stop_at_frame_no(long frame) {
            frame_to_stop = frame;
        }
        ...
        bool set_output(const std::string &file_name, 
                int codec = 0, double frame_rate = 0.0, 
                bool is_color = true) {
            ...
            return writer.open(output_file, codec,
                    frame_rate, get_frame_size(), is_color);
        }
        bool set_output(const std::string &file_name, 
                const std::string &ext, int num_of_digits = 3, 
                int start_index = 0) {
        ...
        int get_codec(char codec[4]) {
            ...
            returned.value = 
                static_cast<int>(capture.get(CV_CAP_PROP_FOURCC));
        ...
        void  video_write(bool write) {
            is_video_write = write;
        }
...
class BGFGsegmentor:public Frameprocessor {
    ...
    public:
        ...
        void process(cv::Mat &frame, cv::Mat &output) {
            ...
            cv::threshold(
                    foreground, output, threshold, 255, cv::THRESH_BINARY);
