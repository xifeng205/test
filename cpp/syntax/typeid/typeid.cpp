 ...
 * Author        : cuiyunpeng
 * Email         : @163.com
 * Create time   : 2020-07-17 10:48
 * Last modified : 2020-07-17 10:48
 * Filename      : typeid.cpp
 * Description   : 
*********************************************************************/
...
{
    int a = 1;
    const std::type_info &t_info = typeid(a);         
    std::cout << t_info.name() << "  |  "
        <<  t_info.hash_code() << std::endl; 

    char b = 2;
    const std::type_info &t_info_c = typeid(b);
    std::cout << t_info_c.name() << "  |  "
        <<  t_info_c.hash_code() << std::endl; 
    
    double c = 3;
    const std::type_info &t_info_d = typeid(c);
    std::cout << t_info_d.name() << "  |  "
        <<  t_info_d.hash_code() << std::endl; 

    const std::type_info &t_info_Base = typeid(Base);
    std::cout << t_info_Base.name() << "  |  " 
        <<  t_info_Base.hash_code() << std::endl; 

    const std::type_info &stcu = typeid(struct Str);
    std::cout << stcu.name() << "  |  "
        <<  stcu.hash_code() << std::endl; 

    std::cout << (typeid(int) == typeid(char) ? "true" : "false")
        << std::endl;
    std::cout << std::boolalpha << (typeid(int) == typeid(int)) 
        << std::endl;
