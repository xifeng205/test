 ...
 * Author        : cuiyunpeng
 * Email         : @163.com
 * Create time   : 2020-07-14 15:57
 * Last modified : 2020-07-14 15:57
 * Filename      : copy_if.cpp
 * Description   : 
*********************************************************************/
//     std::vector<string> names {"A1", "Beth", "Carol", "Dan", "Eve","Fred", "George", "Harry", "Iain", "Joe"};
// opy_if() 算法可以从源序列复制使谓词返回 true 的元素，所以可以把它看作一个过滤器。前两个参数定义源序列的输入迭代器，第三个参数是指向目的序列的第一个位置的输出迭代器，第 4 个参数是一个谓词。会返回一个输出迭代器，它指向最后一个被复制元素的下一个位置。下面是一个使用 copy_if() 的示例：
//
//     std::vector<string> names {"A1", "Beth", "Carol", "Dan", "Eve","Fred", "George", "Harry", "Iain", "Joe"};
...
{
    std::vector<string> names{"Hohf", "Fof", "Ffkkkdf"};
    std::unordered_set<string> more_names{"Jon", "Lon"};
    size_t max_length{4};
    std::copy_if(std::begin(names), std::end(names), 
            std::inserter(more_names, more_names.begin()),
            [max_length] (const string &s) { return (s.length() <= max_length); });
    for (auto i: more_names) std::cout << i << " ";
    std::cout << std::endl;
