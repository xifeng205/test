 ...
 * Author        : cuiyunpeng
 * Email         : @163.com
 * Create time   : 2020-07-14 20:48
 * Last modified : 2020-07-14 20:48
 * Filename      : swap_range.cpp
 * Description   : 
*********************************************************************/
/*
...
{
    std::vector<Name> my_vector{Name{"jdf", "helllo"}, 
        Name{"dsfd", "djfiads"}, Name{"dfsda", "kdsfd"}};     

    std::list<Name> my_list{Name{"dsfid", "djfsd"},
        Name{"djfids", "dfdsfd"}, Name{"dsfjdf", "dfds"}};

    std::swap_ranges(std::begin(my_vector), std::begin(my_vector)+2, 
                std::begin(my_list));
    std::for_each(std::begin(my_vector), std::end(my_vector), 
            [](const Name &name) { std::cout << name.first << ":"
             << name.second;});
    std::cout << std::endl;
