 ...
 * Author        : cuiyunpeng
 * Email         : @163.com
 * Create time   : 2020-07-14 20:09
 * Last modified : 2020-07-14 20:09
 * Filename      : rotate.cpp
 * Description   : 
*********************************************************************/
/*
...
int main(int argc, char **argv)
{
    ...
    std::vector<string> rotate_cp;
    auto iter = std::rotate_copy(start, std::find(std::begin(words), std::end (words), "five") , end_iter, std::back_inserter(rotate_cp));
    std::copy(std::begin(rotate_cp), std::end(rotate_cp), std::ostream_iterator<string>{std::cout, " "});
    // std::cout << "First element before rotation: " << *iter << std::endl;
    return 0;
}
