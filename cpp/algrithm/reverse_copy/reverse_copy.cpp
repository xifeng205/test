...
int main(int argc, char **argv)
{
    // while (true) {
        ...
        std::copy_if(std::begin(s), std::end(s), 
                std::back_inserter(normal_s),
                [](char ch) { return std::isalpha(ch); });
        std::cout << normal_s << std::endl;
        
        std::for_each(std::begin(normal_s), std::end(normal_s),
                [](char &ch) { ch = std::toupper(ch); });

        string reverse_s;
        std::reverse_copy(std::begin(normal_s), 
                std::end(normal_s),
                std::back_inserter(reverse_s));
        std::cout << "Sentence " 
            << ((normal_s == reverse_s) ? "is" : "is not ")
            << " a palindorme." << std::endl;
