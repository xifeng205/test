...
const char *keys = "{ help     h  | | Print help message. }"
                   "{ input    i  | | Path to input image or video file. Skip "
                   "this argument to capture frames from a camera.}"
                   "{ device      |  0  | camera device number. }"
                   "{ modelBin    |     | Path to a binary .caffemodel file "
                   "contains trained network.}"
                   "{ modelTxt    |     | Path to a .prototxt file contains "
                   "the model definition of trained network.}"
