...
static void help(char **argv) {
  cout << "\nThis program demonstrates how to use MSER to detect extremal "
          "regions\n"
          "Usage:\n"
       << argv[0]
       << " <image1(without parameter a synthetic image is used as default)>\n"
          "Press esc key when image window is active to change descriptor "
          "parameter\n"
          "Press 2, 8, 4, 6, +, -, or 5 keys in openGL windows to change view "
          "or use mouse\n";
}
...
static void DrawOpenGLMSER(Mat img, Mat result) {
  ...
  for (int i = 0, nbPix = 0; i < img.rows; i++) {
    for (int j = 0; j < img.cols; j++, nbPix++) {
      ...
      vertex.at<Vec3f>(0, nbPix) =
          Vec3f(float(2 * (x - 0.5)), float(2 * (0.5 - y)),
                float(imgGray.at<uchar>(i, j) / 512.0));
...
static void addNestedRectangles(Mat &img, Point p0, int *width, int *color,
                                int n) {
  for (int i = 0; i < n; i++) {
...
static void addNestedCircles(Mat &img, Point p0, int *width, int *color,
                             int n) {
  for (int i = 0; i < n; i++) {
...
static Mat MakeSyntheticImage() {
  ...
  int width[] = {390, 380, 300, 290, 280, 270, 260,
                 250, 210, 190, 150, 100, 80,  70};

  int color1[] = {80,  180, 160, 140, 120, 100, 90,
                  110, 170, 150, 140, 100, 220};
  int color2[] = {81,  181, 161, 141, 121, 101, 91,
                  111, 171, 151, 141, 101, 221};
...
int main(int argc, char *argv[]) {
  ...
  for (itDesc = typeDesc.begin(); itDesc != typeDesc.end(); ++itDesc) {
    ...
    if (*itDesc == "MSER") {
      if (img.type() == CV_8UC3) {
        b = MSER::create(itMSER->delta, itMSER->minArea, itMSER->maxArea,
                         itMSER->maxVariation, itMSER->minDiversity,
                         itMSER->maxEvolution, itMSER->areaThreshold,
                         itMSER->minMargin, itMSER->edgeBlurSize);
    ...
    try {
      ...
      if (b.dynamicCast<MSER>().get()) {
        ...
        // result = Scalar(0, 0, 0);
        int nbPixelInMSER = 0;
        for (vector<vector<Point>>::iterator itr = region.begin();
             itr != region.end(); ++itr) {
          for (vector<Point>::iterator itp = itr->begin(); itp != itr->end();
               ++itp) {
        ...
        cout << "Number of MSER region: " << region.size()
             << "; Number of pixels in all MSER region: " << nbPixelInMSER
             << "\n";
      }
