...
int main(int argc, char **argv)
{
    ...
    cv::Mat img1 = cv::imread("../../../data/box_in_scene.png");
    ...
    cv::namedWindow("image1 origin");
    cv::imshow("image1 origin", img1);
    cv::Mat result;
    cv::Mat out;

    sharp.get_miliisecond(0);
    sharp.sharpen(img, result);
    cv::addWeighted(img, 0.7, result, 0.1, 0., out);
    duration = sharp.get_miliisecond(1);
    cout << "time cost: " << duration << endl;

    cv::namedWindow("image sharpen");
    cv::imshow("image sharpen", result);
    cv::imshow("image sharpen", out);
    cv::waitKey(10000);
    return 0;
}
