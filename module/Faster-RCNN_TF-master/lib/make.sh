...
if [ -d "$CUDA_PATH" ]; then
	...
	g++ -std=c++11 -shared -o roi_pooling.so roi_pooling_op.cc -D_GLIBCXX_USE_CXX11_ABI=0 \
		roi_pooling_op.cu.o -I $TF_INC -L $TF_LIB -l:libtensorflow_framework.so.2 -D GOOGLE_CUDA=1 -fPIC $CXXFLAGS \
		-lcudart -L $CUDA_PATH/lib64
else
	g++ -std=c++11 -shared -o roi_pooling.so roi_pooling_op.cc \
		-I $TF_INC -L $TF_LIB -l:libtensorflow_framework.so.2 -fPIC $CXXFLAGS -D_GLIBCXX_USE_CXX11_ABI=0
fi
